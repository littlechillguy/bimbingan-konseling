...
<body class="antialiased min-h-screen flex flex-col items-center">
    ...
    <main class="w-full max-w-lg px-6 mt-8 md:mt-12 mb-16 flex-1 flex flex-col">
        ...
        @if(session('success'))
        <div class="mb-6 bg-emerald-50 border border-emerald-100 text-emerald-700 px-4 py-4 rounded-2xl text-[11px] font-bold flex items-start gap-3 fade-in">
        ...
        {{-- Menampilkan Error Validasi & Rate Limiting --}}
        @if ($errors->any())
        <div class="mb-6 bg-red-50 border border-red-100 text-red-600 px-4 py-4 rounded-2xl text-[11px] font-bold fade-in">
        ...
        <div class="relative">
            <form action="{{ route('layanan.chat-anonim.store') }}" method="POST" class="bg-white rounded-[2rem] md:rounded-[2.5rem] border border-gray-100 soft-shadow overflow-hidden focus-within:ring-4 focus-within:ring-teal-50/50 transition-all duration-500">
                ...
                <textarea 
                    name="message"
                    placeholder="Tuliskan pengalaman atau pertanyaanmu di sini..." 
                    rows="6"
                    required
                    class="w-full p-6 md:p-8 text-gray-700 font-medium placeholder-gray-300 bg-transparent border-none focus:ring-0 resize-none text-sm md:text-base outline-none min-h-[180px]"
                ...
                <div class="flex flex-col sm:flex-row items-center justify-between p-4 bg-gray-50/30 border-t border-gray-50 gap-4">
                    <div class="flex flex-col gap-1 pl-2">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">Keamanan Berlapis</span>
                        </div>
                        <span class="text-[8px] text-gray-300 font-bold uppercase tracking-[0.1em]">Auto-Filter & Anti-Spam Aktif</span>
                    </div>
                    
                    <button type="submit" class="w-full sm:w-auto bg-gray-900 text-white px-8 py-4 rounded-xl md:rounded-2xl font-bold text-[10px] uppercase tracking-[0.2em] hover:bg-teal-600 active:scale-95 transition-all duration-300 shadow-xl shadow-gray-200">
                        Kirim Pesan
                    </button>
                </div>
            </form>
    ...
    <footer class="w-full py-8 text-center mt-auto">
        <div class="flex items-center justify-center gap-4">
            ...
            <span class="text-[9px] font-black text-gray-300 uppercase tracking-[0.3em]">Bimbingan Konseling Room</span>
            <div class="h-[1px] w-6 bg-gray-100"></div>
        </div>
    </footer>
