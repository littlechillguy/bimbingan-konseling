<nav x-data="{ open: false, atTop: true, userMenu: false }"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-20 items-center">
            ...
            <a href="{{ route('home') }}" class="flex items-center group outline-none">
                <div class="relative">
                    {{-- Logo Putih Anda --}}
                    <img src="{{ asset('asset/logo43.png') }}" 
                         alt="Logo Sekolah" 
                    ...
                    <div class="absolute inset-0 bg-orange-500/30 blur-xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity"></div>
                </div>
            </a>

            <div class="hidden md:flex items-center space-x-8 text-sm font-bold">
                {{-- Link menu dengan warna putih/abu terang agar kontras --}}
                <a href="{{ route('home') }}" class="text-white hover:text-orange-400 transition drop-shadow-sm">Beranda</a>
                <a href="#layanan" class="text-white hover:text-orange-400 transition drop-shadow-sm">Layanan</a>
                ...
                @auth
                    <div class="relative" @click.away="userMenu = false">
                        <button @click="userMenu = !userMenu" class="flex items-center space-x-3 focus:outline-none group">
                            ...
                            <div class="w-10 h-10 bg-orange-500 border-2 border-white/50 rounded-full flex items-center justify-center text-white shadow-lg overflow-hidden group-hover:border-orange-400 transition">
                        ...
                        <div x-show="userMenu" x-cloak 
                            ...
                            <a href="{{ url('/dashboard') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-orange-50 hover:text-orange-600 transition font-medium">Dashboard</a>
                            <hr class="my-2 border-gray-50">
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition font-bold">
                                    Keluar
                                </button>
                            </form>
                        </div>
    ...
    <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200"
        ...
        <a href="{{ route('home') }}" class="block px-4 py-4 text-base font-bold text-white border-b border-white/5">Beranda</a>
        <a href="#layanan" class="block px-4 py-4 text-base font-bold text-white border-b border-white/5">Layanan</a>
        
        <div class="pt-4">
            @auth
                <a href="{{ url('/dashboard') }}" class="block w-full text-center px-6 py-4 bg-orange-500 text-white rounded-2xl font-bold shadow-lg shadow-orange-500/20">
                    Dashboard Siswa
                </a>
            @else
                <a href="{{ route('login') }}" class="block w-full text-center px-6 py-4 bg-orange-500 text-white rounded-2xl font-bold shadow-lg shadow-orange-500/20">
                    Mulai Konseling
                </a>
            @endauth
        </div>
